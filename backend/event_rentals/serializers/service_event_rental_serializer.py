...
class CreateServiceEventRentalSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ServicesEventRental
        fields = ['service_id', 'service_quantity', 'date_to_deliver', "description", "service_observation"]
    ...
    def validate(self, attrs):
        ...
        if attrs['service_quantity'] < 1:
            raise serializers.ValidationError('La cantidad de servicios debe ser mayor a 0.')
        if ServicesEventRentalService.get_all().filter(eventrental=self.context['event_rental'], service_id=attrs['service_id']).exists():
            raise serializers.ValidationError('El servicio ya está asociado a este alquiler de evento.')
        if attrs['date_to_deliver'] < self.context['event_rental'].event_rental_date:
            raise serializers.ValidationError('La fecha de entrega del servicio no puede ser menor a la fecha de inicio del alquiler de evento.')
        return attrs
